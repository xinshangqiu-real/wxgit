...
typedef struct xss_osal_sema_priv {
    sem_t sema;
} xss_osal_sema_priv_t;


xss_result_t xss_osal_sema_create(xss_osal_sema_t **sema, xss_osal_sema_attr_t *attr)
{
    xss_osal_sema_t *xsema = NULL;
    xss_osal_sema_priv_t *priv = NULL;
    void *mem_base = NULL;
    ...
    mem_base = calloc(sizeof(xss_osal_sema_t) + sizeof(xss_osal_sema_priv_t), 1);
    if (mem_base == NULL) {
        return ERR_NOMEM;
    }

    xsema = (xss_osal_sema_t *)mem_base;
    xsema->priv = mem_base + sizeof(xss_osal_sema_t);
    memcpy(&xsema->attr, attr, sizeof(xss_osal_sema_attr_t));
    priv = (xss_osal_sema_priv_t *)xsema->priv;
...
xss_result_t xss_osal_sema_destroy(xss_osal_sema_t *sema)
{
    ...
    priv = (xss_osal_sema_priv_t *)sema->priv;
