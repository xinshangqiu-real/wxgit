...
typedef struct xss_timer_priv {
	pthread_t thread;				/* Pthread insatnce */
	bool exit;						/* Pthread need exit or not */
...
{
	xss_timer_t *timer = (xss_timer_t *)arg;
	xss_timer_priv_t *priv = (xss_timer_priv_t *)timer->priv;
	xss_timer_attr_t *attr = &timer->attr;
...
{
	xss_timer_t *xtimer = NULL;
	xss_timer_priv_t *priv = NULL;
	...
	mem_base = calloc(sizeof(xss_timer_t) + sizeof(xss_timer_priv_t), 1);
	if (mem_base == NULL) {
		return ERR_NOMEM;
	}

	xtimer = (xss_timer_t *)mem_base;
	memcpy(&xtimer->attr, attr, sizeof(xss_timer_attr_t));
	xtimer->priv = mem_base + sizeof(xss_timer_t);
	priv = (xss_timer_priv_t *)xtimer->priv;
...
xss_result_t xss_timer_destroy(xss_timer_t *timer)
{
	...
	priv = (xss_timer_priv_t *)timer->priv;
