...
{
    xss_mutex_t *xmutex = NULL;
    xss_mutex_priv_t *priv = NULL;
    void *mem_base = NULL;

    mem_base = calloc(sizeof(xss_mutex_t) + sizeof(xss_mutex_priv_t), 1);
    if (mem_base == NULL) {
        return ERR_NOMEM;
    }

    xmutex = (xss_mutex_t *)mem_base;
    xmutex->priv = mem_base + sizeof(xss_mutex_t);
    memcpy(&xmutex->attr, attr, sizeof(xss_mutex_attr_t));
    priv = (xss_mutex_priv_t *)xmutex->priv;
...
xss_result_t xss_mutex_destroy(xss_mutex_t *mutex)
{
    ...
    priv = (xss_mutex_priv_t *)mutex->priv;
